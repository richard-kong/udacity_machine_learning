...
def test_classifier(clf, dataset, feature_list, folds = 1000):
    ...
    for train_idx, test_idx in cv: 
        ...
        # record the feature ranking

        for f in range(len(importances)):
            feature_importance[indices[f]].append(importances[indices[f]])
    ...
    try:
        ...
        for feature in feature_importance:
          average_importance =  sum(feature_importance[feature]) /  len(feature_importance[feature])
          print("feature: %s (%f)" % (feature_list[feature + 1], average_importance))          
